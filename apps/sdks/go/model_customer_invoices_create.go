/*
lomi. API

Payment processing API for francophone West African businesses.

API version: 1.1.0
Contact: hello@lomi.africa
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package lomisdk

import (
	"encoding/json"
)

// checks if the CustomerInvoicesCreate type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CustomerInvoicesCreate{}

// CustomerInvoicesCreate Request body for creating a customer invoices object. System-managed fields like `created_at`, `organization_id`, and IDs are automatically set.
type CustomerInvoicesCreate struct {
	// Amount in the smallest currency unit (e.g., cents for USD, same for XOF)
	Amount *float64 `json:"amount,omitempty"`
	// Three-letter ISO currency code (e.g., XOF, USD, EUR)
	CurrencyCode *string `json:"currency_code,omitempty"`
	// Unique identifier (UUID format)
	CustomerId *string `json:"customer_id,omitempty"`
	// Unique identifier (UUID format)
	CustomerInvoiceId *string `json:"customer_invoice_id,omitempty"`
	Description *string `json:"description,omitempty"`
	DueDate *string `json:"due_date,omitempty"`
	// Set of key-value pairs for storing additional information
	Metadata map[string]interface{} `json:"metadata,omitempty"`
	// Current status of the resource
	Status *string `json:"status,omitempty"`
}

// NewCustomerInvoicesCreate instantiates a new CustomerInvoicesCreate object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCustomerInvoicesCreate() *CustomerInvoicesCreate {
	this := CustomerInvoicesCreate{}
	return &this
}

// NewCustomerInvoicesCreateWithDefaults instantiates a new CustomerInvoicesCreate object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCustomerInvoicesCreateWithDefaults() *CustomerInvoicesCreate {
	this := CustomerInvoicesCreate{}
	return &this
}

// GetAmount returns the Amount field value if set, zero value otherwise.
func (o *CustomerInvoicesCreate) GetAmount() float64 {
	if o == nil || IsNil(o.Amount) {
		var ret float64
		return ret
	}
	return *o.Amount
}

// GetAmountOk returns a tuple with the Amount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CustomerInvoicesCreate) GetAmountOk() (*float64, bool) {
	if o == nil || IsNil(o.Amount) {
		return nil, false
	}
	return o.Amount, true
}

// HasAmount returns a boolean if a field has been set.
func (o *CustomerInvoicesCreate) HasAmount() bool {
	if o != nil && !IsNil(o.Amount) {
		return true
	}

	return false
}

// SetAmount gets a reference to the given float64 and assigns it to the Amount field.
func (o *CustomerInvoicesCreate) SetAmount(v float64) {
	o.Amount = &v
}

// GetCurrencyCode returns the CurrencyCode field value if set, zero value otherwise.
func (o *CustomerInvoicesCreate) GetCurrencyCode() string {
	if o == nil || IsNil(o.CurrencyCode) {
		var ret string
		return ret
	}
	return *o.CurrencyCode
}

// GetCurrencyCodeOk returns a tuple with the CurrencyCode field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CustomerInvoicesCreate) GetCurrencyCodeOk() (*string, bool) {
	if o == nil || IsNil(o.CurrencyCode) {
		return nil, false
	}
	return o.CurrencyCode, true
}

// HasCurrencyCode returns a boolean if a field has been set.
func (o *CustomerInvoicesCreate) HasCurrencyCode() bool {
	if o != nil && !IsNil(o.CurrencyCode) {
		return true
	}

	return false
}

// SetCurrencyCode gets a reference to the given string and assigns it to the CurrencyCode field.
func (o *CustomerInvoicesCreate) SetCurrencyCode(v string) {
	o.CurrencyCode = &v
}

// GetCustomerId returns the CustomerId field value if set, zero value otherwise.
func (o *CustomerInvoicesCreate) GetCustomerId() string {
	if o == nil || IsNil(o.CustomerId) {
		var ret string
		return ret
	}
	return *o.CustomerId
}

// GetCustomerIdOk returns a tuple with the CustomerId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CustomerInvoicesCreate) GetCustomerIdOk() (*string, bool) {
	if o == nil || IsNil(o.CustomerId) {
		return nil, false
	}
	return o.CustomerId, true
}

// HasCustomerId returns a boolean if a field has been set.
func (o *CustomerInvoicesCreate) HasCustomerId() bool {
	if o != nil && !IsNil(o.CustomerId) {
		return true
	}

	return false
}

// SetCustomerId gets a reference to the given string and assigns it to the CustomerId field.
func (o *CustomerInvoicesCreate) SetCustomerId(v string) {
	o.CustomerId = &v
}

// GetCustomerInvoiceId returns the CustomerInvoiceId field value if set, zero value otherwise.
func (o *CustomerInvoicesCreate) GetCustomerInvoiceId() string {
	if o == nil || IsNil(o.CustomerInvoiceId) {
		var ret string
		return ret
	}
	return *o.CustomerInvoiceId
}

// GetCustomerInvoiceIdOk returns a tuple with the CustomerInvoiceId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CustomerInvoicesCreate) GetCustomerInvoiceIdOk() (*string, bool) {
	if o == nil || IsNil(o.CustomerInvoiceId) {
		return nil, false
	}
	return o.CustomerInvoiceId, true
}

// HasCustomerInvoiceId returns a boolean if a field has been set.
func (o *CustomerInvoicesCreate) HasCustomerInvoiceId() bool {
	if o != nil && !IsNil(o.CustomerInvoiceId) {
		return true
	}

	return false
}

// SetCustomerInvoiceId gets a reference to the given string and assigns it to the CustomerInvoiceId field.
func (o *CustomerInvoicesCreate) SetCustomerInvoiceId(v string) {
	o.CustomerInvoiceId = &v
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *CustomerInvoicesCreate) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CustomerInvoicesCreate) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *CustomerInvoicesCreate) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *CustomerInvoicesCreate) SetDescription(v string) {
	o.Description = &v
}

// GetDueDate returns the DueDate field value if set, zero value otherwise.
func (o *CustomerInvoicesCreate) GetDueDate() string {
	if o == nil || IsNil(o.DueDate) {
		var ret string
		return ret
	}
	return *o.DueDate
}

// GetDueDateOk returns a tuple with the DueDate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CustomerInvoicesCreate) GetDueDateOk() (*string, bool) {
	if o == nil || IsNil(o.DueDate) {
		return nil, false
	}
	return o.DueDate, true
}

// HasDueDate returns a boolean if a field has been set.
func (o *CustomerInvoicesCreate) HasDueDate() bool {
	if o != nil && !IsNil(o.DueDate) {
		return true
	}

	return false
}

// SetDueDate gets a reference to the given string and assigns it to the DueDate field.
func (o *CustomerInvoicesCreate) SetDueDate(v string) {
	o.DueDate = &v
}

// GetMetadata returns the Metadata field value if set, zero value otherwise.
func (o *CustomerInvoicesCreate) GetMetadata() map[string]interface{} {
	if o == nil || IsNil(o.Metadata) {
		var ret map[string]interface{}
		return ret
	}
	return o.Metadata
}

// GetMetadataOk returns a tuple with the Metadata field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CustomerInvoicesCreate) GetMetadataOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.Metadata) {
		return map[string]interface{}{}, false
	}
	return o.Metadata, true
}

// HasMetadata returns a boolean if a field has been set.
func (o *CustomerInvoicesCreate) HasMetadata() bool {
	if o != nil && !IsNil(o.Metadata) {
		return true
	}

	return false
}

// SetMetadata gets a reference to the given map[string]interface{} and assigns it to the Metadata field.
func (o *CustomerInvoicesCreate) SetMetadata(v map[string]interface{}) {
	o.Metadata = v
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *CustomerInvoicesCreate) GetStatus() string {
	if o == nil || IsNil(o.Status) {
		var ret string
		return ret
	}
	return *o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CustomerInvoicesCreate) GetStatusOk() (*string, bool) {
	if o == nil || IsNil(o.Status) {
		return nil, false
	}
	return o.Status, true
}

// HasStatus returns a boolean if a field has been set.
func (o *CustomerInvoicesCreate) HasStatus() bool {
	if o != nil && !IsNil(o.Status) {
		return true
	}

	return false
}

// SetStatus gets a reference to the given string and assigns it to the Status field.
func (o *CustomerInvoicesCreate) SetStatus(v string) {
	o.Status = &v
}

func (o CustomerInvoicesCreate) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CustomerInvoicesCreate) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Amount) {
		toSerialize["amount"] = o.Amount
	}
	if !IsNil(o.CurrencyCode) {
		toSerialize["currency_code"] = o.CurrencyCode
	}
	if !IsNil(o.CustomerId) {
		toSerialize["customer_id"] = o.CustomerId
	}
	if !IsNil(o.CustomerInvoiceId) {
		toSerialize["customer_invoice_id"] = o.CustomerInvoiceId
	}
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	if !IsNil(o.DueDate) {
		toSerialize["due_date"] = o.DueDate
	}
	if !IsNil(o.Metadata) {
		toSerialize["metadata"] = o.Metadata
	}
	if !IsNil(o.Status) {
		toSerialize["status"] = o.Status
	}
	return toSerialize, nil
}

type NullableCustomerInvoicesCreate struct {
	value *CustomerInvoicesCreate
	isSet bool
}

func (v NullableCustomerInvoicesCreate) Get() *CustomerInvoicesCreate {
	return v.value
}

func (v *NullableCustomerInvoicesCreate) Set(val *CustomerInvoicesCreate) {
	v.value = val
	v.isSet = true
}

func (v NullableCustomerInvoicesCreate) IsSet() bool {
	return v.isSet
}

func (v *NullableCustomerInvoicesCreate) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCustomerInvoicesCreate(val *CustomerInvoicesCreate) *NullableCustomerInvoicesCreate {
	return &NullableCustomerInvoicesCreate{value: val, isSet: true}
}

func (v NullableCustomerInvoicesCreate) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCustomerInvoicesCreate) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


