...
func main() {
	...
	resp, r, err := apiClient.BNPLAPI.ListInstallmentPayments(context.Background()).Limit(limit).Offset(offset).Sort(sort).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `BNPLAPI.ListInstallmentPayments``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ListInstallmentPayments`: ListInstallmentPayments200Response
	fmt.Fprintf(os.Stdout, "Response from `BNPLAPI.ListInstallmentPayments`: %v\n", resp)
}
```
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.BNPLAPI.RetrieveInstallmentPayment(context.Background(), paymentId).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `BNPLAPI.RetrieveInstallmentPayment``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `RetrieveInstallmentPayment`: InstallmentPayments
	fmt.Fprintf(os.Stdout, "Response from `BNPLAPI.RetrieveInstallmentPayment`: %v\n", resp)
}
```
