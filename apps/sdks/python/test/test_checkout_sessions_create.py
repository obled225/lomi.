# coding: utf-8

"""
    lomi. API

    # lomi. API  The lomi. API provides a comprehensive payment processing platform for francophone WestAfrican businesses.  ## Authentication  All API requests require authentication using an API key. Include your API key in the `X-API-KEY` header.  ```bash curl https://api.lomi.africa/v1/customers \\   -H \"X-API-KEY: your_api_key_here\" ```  ## Rate limiting  API requests are rate-limited based on your plan. Rate limit headers are included in all responses.  ## Webhooks  Configure webhooks to receive real-time notifications about events in your account.  ## Environments  - **Live**: Production environment with real transactions - **Test**: Sandbox environment for development and testing

    The version of the OpenAPI document: 1.1.0
    Contact: hello@lomi.africa
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from lomi_sdk.models.checkout_sessions_create import CheckoutSessionsCreate

class TestCheckoutSessionsCreate(unittest.TestCase):
    """CheckoutSessionsCreate unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> CheckoutSessionsCreate:
        """Test CheckoutSessionsCreate
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `CheckoutSessionsCreate`
        """
        model = CheckoutSessionsCreate()
        if include_optional:
            return CheckoutSessionsCreate(
                allow_coupon_code = True,
                allow_quantity = True,
                allowed_providers = '',
                amount = 1.337,
                cancel_url = '',
                checkout_session_id = '',
                created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                created_by = '',
                currency_code = '',
                customer_email = '',
                customer_id = '',
                customer_name = '',
                customer_phone = '',
                environment = '',
                expires_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                installment_plan_id = '',
                is_pos = True,
                is_spi = True,
                metadata = None,
                organization_id = '',
                payment_link_id = '',
                payment_request_id = '',
                price_id = '',
                product_id = '',
                public_description = '',
                qr_code_data = None,
                qr_code_type = '',
                quantity = 1.337,
                spi_account_number = '',
                spi_qr_code_id = '',
                status = '',
                subscription_id = '',
                success_url = '',
                title = '',
                updated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f')
            )
        else:
            return CheckoutSessionsCreate(
        )
        """

    def testCheckoutSessionsCreate(self):
        """Test CheckoutSessionsCreate"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
