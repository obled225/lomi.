...
const schemaPath = resolve(
  join(__dirname, '../../dashboard/supabase/migrations/20240828000002_db.sql'),
);
const schemaContent = readFileSync(schemaPath, 'utf8');

console.log(
  '🔍 Verifying type consistency between SQL schema and TypeScript...',
);
...
function verifyEnums(sqlEnums: Record<string, string[]>): void {
  ...
  Object.entries(sqlToTsEnumMap).forEach(([sqlName, tsName]) => {
    ...
    if (!tsEnum) {
      console.error(
        `❌ TypeScript enum ${tsName} not found for SQL enum ${sqlName}`,
      );
      inconsistencies++;
      return;
    }
    ...
    if (missingInTs.length > 0) {
      console.error(
        `❌ Values missing in TypeScript enum ${tsName}: ${missingInTs.join(', ')}`,
      );
      inconsistencies++;
    }
    ...
    if (extraInTs.length > 0) {
      console.error(
        `❌ Extra values in TypeScript enum ${tsName} not in SQL: ${extraInTs.join(', ')}`,
      );
      inconsistencies++;
    }
  ...
  if (inconsistencies === 0) {
    ...
    console.error(
      `❌ Found ${inconsistencies} inconsistencies between SQL and TypeScript enums`,
    );
  }
}
...
function verifyInterfaces(): void {
  ...
  console.log(
    '⚠️ Interface validation with tables requires additional implementation',
  );
}
