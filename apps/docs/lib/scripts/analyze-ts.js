...
const LOCALES = COMMAND === 'check' || COMMAND === 'populate'
    ? languages.map(lang => lang.code)
    : (process.argv.length > 3 ? process.argv.slice(3) : languages.map(lang => lang.code));
...
async function analyzeTranslations() {
    ...
    for (const locale of LOCALES) {
        ...
        console.log(`🛡️  Protected ${allDefinedKeys.size - unusedKeys.size} keys (including dynamic prefixes)`);
        ...
        console.log(`✅ Updated ${locale.toUpperCase()} file: ${path.relative(process.cwd(), localeFile)}`);
        ...
        console.log(`  - Savings: ${((unusedKeys.size / allDefinedKeys.size) * 100).toFixed(1)}%\n`);
    }

    console.log('🎉 All locales processed! Original files have been cleaned and sorted.');
    console.log('\n🔒 Dynamic prefixes protected:');
    Array.from(dynamicPrefixes).sort().forEach(prefix => {
        console.log(`  - ${prefix}*`);
    });
}
...
async function checkTranslationCompleteness() {
    ...
    for (const locale of ['fr', 'es', 'zh']) {
        ...
        if (missingCount > 0) {
            console.log(`   - Missing keys (${missingCount}):`);
            Array.from(missingKeys).sort().slice(0, 20).forEach(key => {
                console.log(`     • ${key}`);
            });
            ...
            const missingContent = `Missing translation keys in ${locale.toUpperCase()} (${missingCount} keys):\n\n` +
                Array.from(missingKeys).sort().map(key => key).join('\n');

            fs.writeFileSync(missingFile, missingContent);
            console.log(`   - Missing keys saved to: ${path.relative(process.cwd(), missingFile)}`);
        }
        console.log('');
    }
...
if (COMMAND === 'check') {
    ...
    console.log('   node analyze-i18n.js clean [locales...]  - Clean unused keys');
    console.log('   node analyze-i18n.js check               - Check completeness vs EN');
    process.exit(1);
}
