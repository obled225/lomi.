...
6.  **Secure Storage**: The CLI saves this **CLI Token** securely in the global configuration file (typically `~/.lomi/config.json`). This token is used for subsequent API calls made _by the CLI itself_.

<Callout type="info" emoji="💡" title="Security Benefit">
  This device flow avoids the need for you to manually handle or store your main
  `LOMI_API_KEY` for general CLI operations, enhancing the security of your
  primary credentials.
</Callout>
