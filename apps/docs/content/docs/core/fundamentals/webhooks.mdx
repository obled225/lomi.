...
The JSON payload will have the following structure. The `data` object typically contains the relevant resource (e.g., Transaction, Subscription) with internal fields removed.
...
- **Secure Your Secret:** Keep your webhook signing secret confidential. Do not commit it to version control. Use environment variables or a secrets manager (see [API Keys guide](/docs/core/fundamentals/api-keys)).
