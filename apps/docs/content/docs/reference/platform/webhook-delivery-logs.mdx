...
```bash
# List all logs for a webhook
curl -X GET "https://api.lomi.africa/v1/webhook-delivery-logs?webhookId=wh_abc123..." \
  -H "X-API-KEY: $LOMI_API_KEY"

# List failed deliveries only
curl -X GET "https://api.lomi.africa/v1/webhook-delivery-logs?webhookId=wh_abc123...&failedOnly=true" \
  -H "X-API-KEY: $LOMI_API_KEY"
```
</Tab>
...
<Tab value="cURL">
```bash
curl -X GET "https://api.lomi.africa/v1/webhook-delivery-logs/log_abc123..." \
  -H "X-API-KEY: $LOMI_API_KEY"
```
</Tab>
