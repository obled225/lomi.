...
<Tab value="cURL">
```bash
curl -X GET "https://api.lomi.africa/v1/transactions?status=completed&provider=WAVE&startDate=2024-01-01T00:00:00Z&page=1&pageSize=50" \
...
<Tab value="cURL">
```bash
curl -X GET "https://api.lomi.africa/v1/transactions/tx_abc123..." \
  -H "X-API-KEY: $LOMI_API_KEY"
```
</Tab>
